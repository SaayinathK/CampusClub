...
const EVENTS_HERO_IMAGE = 'https://images.unsplash.com/photo-1511578314322-379afb476865?q=80&w=2070&auto=format&fit=crop';
   ...
   return (
      <div className="group relative glass-dark rounded-3xl overflow-hidden border border-slate-200 hover:border-blue-300 transition-all duration-500 hover:-translate-y-2 shadow-2xl flex flex-col">
         ...
         <div className="p-6 flex flex-col flex-1 bg-white/80">
            ...
            <div className="flex flex-col gap-2 mb-5 text-[11px] text-slate-600 font-bold uppercase tracking-widest">
            ...
            <div className="mt-auto flex gap-3">
               <Link
                  to={`/events/${event._id}`}
                  className="flex-1 text-center py-3 rounded-xl bg-white hover:bg-blue-50 text-slate-900 text-[10px] font-black transition-all active:scale-95 uppercase tracking-[0.2em] border border-slate-200 flex items-center justify-center gap-2 group/btn"
               >
...
const FeaturedCard = ({ event }) => (
   <div className="relative group rounded-[2.5rem] overflow-hidden min-h-[420px] flex items-end shadow-2xl shadow-blue-500/10 border border-slate-200 hover:border-blue-300 transition-all duration-700">
      {event.coverImage ? (
      ...
      <div className="relative p-10 w-full">
         <div className="flex justify-between items-start mb-4">
            <span className="inline-block px-4 py-1.5 rounded-xl bg-blue-600 font-black text-white uppercase tracking-[0.3em] text-[10px] shadow-2xl shadow-blue-500/25">
               Featured Event
            </span>
            <CategoryBadge category={event.category} />
         </div>
         <h3 className="text-3xl md:text-4xl font-black mb-4 uppercase leading-tight tracking-tighter text-slate-900">{event.title}</h3>
         ...
         <Link
            to={`/events/${event._id}`}
            className="inline-flex items-center gap-2 px-8 py-3 rounded-2xl bg-slate-900 text-white font-black text-[11px] transition-all hover:bg-slate-800 active:scale-95 shadow-2xl uppercase tracking-[0.3em]"
         >
...
export default function Events() {
   ...
   return (
      <div className="relative pt-32 pb-24 min-h-screen text-slate-900 overflow-hidden">
         <div className="absolute inset-x-0 top-0 h-[420px] pointer-events-none">
            <img
               src={EVENTS_HERO_IMAGE}
               alt="Campus event crowd"
               className="h-full w-full object-cover opacity-20"
            />
            <div className="absolute inset-0 bg-gradient-to-b from-indigo-100/60 via-white/65 to-transparent" />
         </div>
         <div className="absolute top-24 -right-24 h-80 w-80 rounded-full bg-indigo-400/20 blur-3xl pointer-events-none" />
         <div className="absolute top-52 -left-24 h-72 w-72 rounded-full bg-sky-400/20 blur-3xl pointer-events-none" />
         <div className="container mx-auto px-6 relative z-10">
            ...
            {/* Search + Filter */}
            <motion.div initial={rise.initial} animate={rise.animate} transition={{ duration: 0.55, delay: 0.08 }} className="flex flex-wrap gap-3 mb-12">
               <input
                  type="text"
                  placeholder="Search events..."
                  value={search}
                  onChange={e => setSearch(e.target.value)}
                  className="theme-input rounded-2xl px-5 py-3 text-sm min-w-[220px]"
               />
               <button
                  onClick={() => setCategory('')}
                  className={`px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border ${category === '' ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-600 border-slate-200 hover:border-blue-300'}`}
               >
                  All
               </button>
               {CATEGORIES.map(c => (
                  <button
                     ...
                     className={`px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border ${category === c ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-600 border-slate-200 hover:border-blue-300'}`}
                  >
                     {c}
                  </button>
               ))}
            ...
            {loading ? (
               ...
               <div className="text-center py-40 surface-panel rounded-[3rem] border border-slate-200 max-w-4xl mx-auto shadow-2xl">
               ...
               <>
                  {/* Featured */}
                  {featured.length > 0 && (
                     <motion.section initial={rise.initial} animate={rise.animate} transition={{ duration: 0.55, delay: 0.12 }} className="mb-20">
                        <div className="flex items-center gap-6 mb-10">
                           ...
                           <div className="flex items-center gap-2 px-4 py-2 rounded-full glass border border-slate-200 text-blue-600 text-[10px] font-black uppercase tracking-widest whitespace-nowrap">
                              <Star size={12} strokeWidth={3} /> Featured
                           </div>
                        </div>
                        <div className="grid md:grid-cols-2 gap-8">
                           {featured.map((ev, idx) => (
                              <motion.div key={ev._id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: idx * 0.06 }}>
                  ...
                  {rest.length > 0 && (
                     <motion.section initial={rise.initial} animate={rise.animate} transition={{ duration: 0.55, delay: 0.18 }}>
                        <div className="flex items-center gap-6 mb-10">
                           ...
                           <span className="text-slate-500 text-[10px] font-black uppercase tracking-widest whitespace-nowrap">{events.length} total</span>
                        </div>
                        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                           {rest.map((ev, idx) => (
                              <motion.div key={ev._id} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: idx * 0.04 }}>
                                 <EventCard event={ev} onViewFeedback={handleViewFeedback} />
                              </motion.div>
                           ))}
                        </div>
                     </motion.section>
