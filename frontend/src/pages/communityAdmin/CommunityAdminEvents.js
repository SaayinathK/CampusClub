...
export default function CommunityAdminEvents() {
   ...
   return (
      <div className="min-h-screen p-6 md:p-10 font-sans text-slate-900 relative overflow-hidden bg-transparent">
               ...
               <div className="flex items-center gap-5 md:gap-6 relative z-10 min-w-0">
                  ...
                  <div className="min-w-0">
                     <h2 className="text-3xl md:text-5xl font-black mb-2 tracking-tight leading-none">
                        EVENT <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500">MANAGEMENT</span>
                     </h2>
            ...
            <div className="surface-panel rounded-[2rem] p-2 md:p-3 border border-slate-200 flex flex-wrap gap-2 shadow-sm">
               {[
                  ...
                  <button
                     ...
                     className={`flex-1 min-w-[120px] px-4 py-3 rounded-xl font-bold text-sm tracking-wide transition-all border flex items-center justify-center gap-2 ${statusFilter === f.value ? `${f.bg} border-slate-200 shadow-sm scale-[1.02]` : 'border-transparent text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}
                  >
                     <span>{f.icon}</span> {f.label}
                     {f.value && (
                        <span className="bg-white px-2 py-0.5 rounded-full text-[10px] ml-1 border border-slate-200 shadow-sm">
                        ...
                        <div key={ev._id} className="surface-card border border-slate-200 rounded-3xl overflow-hidden shadow-2xl hover:shadow-[0_20px_50px_rgba(148,163,184,0.18)] transition-all duration-300 hover:-translate-y-1 group relative flex flex-col pt-1">
                           ...
                           <div className="p-6 flex flex-col h-full">
                              ...
                              <div className="flex items-start gap-4 mb-4">
                                 <div className="w-14 h-14 rounded-2xl bg-white border border-slate-200 flex items-center justify-center text-2xl shrink-0 group-hover:scale-110 transition-transform shadow-lg">
                              ...
                              {isRejected && ev.rejectionReason && (
                                 <div className="bg-rose-50 border-l-2 border-rose-500 border border-rose-200 p-3 rounded mb-4 text-xs text-rose-700 leading-tight">
                              ...
                              <div className="space-y-2 mt-auto">
                                 ...
                                 <div className="flex gap-2">
                                    {isRejected && (
                                       <button onClick={() => openEdit(ev)} className="w-full py-2 rounded-xl bg-amber-50 border border-amber-200 font-bold text-xs uppercase tracking-widest text-amber-700 hover:bg-amber-100 transition-colors shadow-sm">
                                          ✏️ Edit & Re-submit
                                       </button>
                                    )}
                                    {isPublished && (
                                    ...
                                    {isPending && (
                                       <button onClick={() => updateStatus(ev._id, 'cancelled')} className="w-full py-2 rounded-xl bg-rose-50 border border-rose-200 font-bold text-xs uppercase tracking-widest text-rose-700 hover:bg-rose-100 transition-colors shadow-sm">
                                          🚫 Cancel Submission
                                       </button>
                                    )}
                                    {['draft', 'cancelled', 'completed'].includes(ev.status) && (
                                       <button onClick={() => deleteEvent(ev._id, ev.title)} className="w-full py-2 rounded-xl bg-white border border-rose-200 font-bold text-xs text-rose-700 hover:bg-rose-500 hover:text-white transition-colors uppercase tracking-widest shadow-sm">
                                          🗑 Delete permanently
                                       </button>
                                    )}
                                 </div>
               ...
               <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
                  <div className="p-8 border-b border-slate-200 sticky top-0 bg-white/95 backdrop-blur-xl z-10 flex justify-between items-center">
                     <div>
                        <h3 className="text-2xl font-black text-slate-900 flex items-center gap-2">
                           {editingEvent ? <span className="text-cyan-600">✏️ EDIT EVENT</span> : <span className="text-blue-600">➕ NEW EVENT</span>}
                        </h3>
                     ...
                     <button onClick={() => setShowModal(false)} className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-rose-500 hover:text-white transition-colors">✕</button>
                  </div>
                  ...
                  <form onSubmit={handleSave} className="p-8 space-y-6">
                     ...
                     <div className="space-y-2 group">
                        ...
                        <input required value={form.title} onChange={e => set('title', e.target.value)} className="theme-input w-full px-4 py-3" placeholder="E.g. Annual Tech Symposium" />
                     </div>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2 group">
                           ...
                           <select value={form.category} onChange={e => set('category', e.target.value)} className="theme-input w-full px-4 py-3 appearance-none cursor-pointer">
                        ...
                        <div className="space-y-2 group">
                           ...
                           <input required type="date" value={form.startDate} onChange={e => set('startDate', e.target.value)} className="theme-input w-full px-4 py-3 [color-scheme:light]" />
                        </div>
                     </div>
                     ...
                     <div className="space-y-2 group">
                        ...
                        <textarea required rows={4} value={form.description} onChange={e => set('description', e.target.value)} className="theme-input w-full px-4 py-3 resize-y" placeholder="Detail the agenda, speakers, requirements..." />
                     </div>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2 group">
                           ...
                           <input value={form.venue} onChange={e => set('venue', e.target.value)} className="theme-input w-full px-4 py-3" placeholder="E.g. Main Auditorium" />
                        </div>
                        <div className="space-y-2 group">
                           ...
                           <input type="number" value={form.maxParticipants} onChange={e => set('maxParticipants', e.target.value)} className="theme-input w-full px-4 py-3" placeholder="Leave empty for unlimited" />
                        </div>
                     </div>
                     ...
                     <div className="space-y-2 group">
                        ...
                        <input
                           ...
                           className="theme-input w-full px-4 py-3 [color-scheme:light]"
                        />
                     </div>
                     ...
                     <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-4">
                        <label className="flex items-center gap-3 cursor-pointer group">
                           <input type="checkbox" checked={!form.isFree} onChange={e => set('isFree', !e.target.checked)} className="w-5 h-5 rounded border-slate-300 bg-white text-yellow-500 focus:ring-yellow-500" />
                           <span className="text-sm font-bold text-slate-700 group-hover:text-slate-900 transition-colors">💳 Paid Event</span>
                        </label>
                        {!form.isFree && (
                           <div className="space-y-2 pl-8">
                              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Ticket Price (LKR)</label>
                              <input
                                 type="number"
                                 min="0"
                                 step="0.01"
                     ...
                     <div className="flex flex-col sm:flex-row gap-4 p-4 rounded-xl bg-slate-50 border border-slate-200">
                        <label className="flex items-center gap-3 cursor-pointer group">
                           <input type="checkbox" checked={form.isVirtual} onChange={e => set('isVirtual', e.target.checked)} className="w-5 h-5 rounded border-slate-300 bg-white text-cyan-500 focus:ring-cyan-500" />
                           <span className="text-sm font-bold text-slate-700 group-hover:text-slate-900 transition-colors">💻 Virtual Event</span>
                        </label>
                        <label className="flex items-center gap-3 cursor-pointer group">
                           <input type="checkbox" checked={form.allowExternal} onChange={e => set('allowExternal', e.target.checked)} className="w-5 h-5 rounded border-slate-300 bg-white text-cyan-500 focus:ring-cyan-500" />
                     ...
                     {form.isVirtual && (
                        <div className="space-y-2 group animate-fade-in">
                           ...
                           <input value={form.virtualLink} onChange={e => set('virtualLink', e.target.value)} className="theme-input w-full px-4 py-3 font-mono text-sm" placeholder="https://zoom.us/..." />
                        </div>
                     )}

                     <div className="pt-6 border-t border-slate-200 flex gap-4 justify-end">
                        <button type="button" onClick={() => setShowModal(false)} className="px-6 py-3 rounded-xl font-bold text-sm text-slate-700 theme-button-secondary transition-colors uppercase tracking-wider">Cancel</button>
                        <button type="submit" disabled={saving} className="px-8 py-3 rounded-xl font-bold text-sm text-white theme-button-primary transition-all uppercase tracking-wider disabled:opacity-50 shadow-[0_8px_25px_rgba(14,165,233,0.22)]">
         ...
         {/* Attendance Modal */}
         {attendanceEvent && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/35 backdrop-blur-sm" onClick={e => e.target === e.currentTarget && setAttendanceEvent(null)}>
               <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-lg max-h-[85vh] overflow-hidden shadow-2xl flex flex-col">

                  <div className="p-6 border-b border-slate-200 bg-white/95 backdrop-blur-xl flex justify-between items-start sticky top-0">
                     <div>
                     ...
                     <button onClick={() => setAttendanceEvent(null)} className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-rose-500 hover:text-white transition-colors">✕</button>
                  </div>
                  ...
                  {/* QR Scanner input */}
                  <div className="p-3 border-b border-slate-200 bg-cyan-50/60">
                     <input
                        type="text"
                        value={qrScanInput}
                        ...
                        className="theme-input w-full px-4 py-2 text-xs font-mono"
                     />
                  </div>

                  <div className="flex gap-2 p-3 border-b border-slate-200 bg-slate-50">
                     <button disabled={attendanceSaving} onClick={() => bulkAttendance(true)} className="flex-1 py-2 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 font-bold text-xs uppercase tracking-widest hover:bg-emerald-100 transition-colors disabled:opacity-50 shadow-sm">
                        ✓ Mark All Present
                     </button>
                     <button disabled={attendanceSaving} onClick={() => bulkAttendance(false)} className="flex-1 py-2 rounded-xl bg-rose-50 border border-rose-200 text-rose-700 font-bold text-xs uppercase tracking-widest hover:bg-rose-100 transition-colors disabled:opacity-50 shadow-sm">
                        ✗ Mark All Absent
                     </button>
                     <button onClick={() => handleExport(attendanceEvent._id, attendanceEvent.title)} className="py-2 px-3 rounded-xl bg-cyan-50 border border-cyan-200 text-cyan-700 font-bold text-xs uppercase tracking-widest hover:bg-cyan-100 transition-colors shadow-sm" title="Export CSV">
                        ⬇ CSV
                     </button>
                  </div>
                  ...
                  <div className="overflow-y-auto p-4 flex flex-col gap-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                     {attendanceList.length === 0 ? (
                        ...
                        attendanceList.map(p => (
                           <div key={p._id} className={`p-4 rounded-2xl border transition-colors ${p.attended ? 'bg-emerald-50 border-emerald-200' : 'bg-white border-slate-200'}`}>
                              <div className="flex items-center gap-4">
                                 <div className={`w-10 h-10 rounded-full flex items-center justify-center text-slate-900 font-bold text-lg shrink-0 ${p.attended ? 'bg-emerald-100 border border-emerald-200' : 'bg-slate-100 border border-slate-200'}`}>
                                 ...
                                 <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-xs text-slate-500 truncate">{p.user?.email || p.externalEmail}</p>
                                    {p.user?.itNumber && <p className="text-[10px] font-mono text-cyan-700 mt-0.5">{p.user.itNumber}</p>}
                                 ...
                                 <div className="flex items-center gap-2 shrink-0">
                                    <button
                                       ...
                                       className="w-8 h-8 rounded-xl bg-purple-50 border border-purple-200 text-purple-700 text-xs flex items-center justify-center hover:bg-purple-100 transition-colors shadow-sm"
                                       title="Show QR code"
                                    >
                                       QR
                                    </button>
                                    ...
                                    <button
                                       ...
                                       className={`w-9 h-9 rounded-xl border font-bold text-sm transition-all hover:scale-105 ${p.attended ? 'bg-emerald-500 border-emerald-500 text-white shadow-[0_0_12px_rgba(16,185,129,0.2)]' : 'bg-white border-slate-200 text-slate-500 hover:border-emerald-300 hover:text-emerald-700'}`}
                              ...
                              {showQR[p._id] && (
                                 <div className="mt-3 flex items-center gap-4 p-3 bg-white rounded-2xl w-fit border border-slate-200 shadow-sm">
         ...
         {selectedEvent && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={e => e.target === e.currentTarget && setSelectedEvent(null)}>
               <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-lg max-h-[80vh] overflow-hidden shadow-2xl flex flex-col">

                  <div className="p-6 border-b border-slate-200 bg-white/95 backdrop-blur-xl flex justify-between items-start">
                     <div>
                     ...
                     <button onClick={() => setSelectedEvent(null)} className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-rose-500 hover:text-white transition-colors">✕</button>
                  </div>
                  ...
                  <div className="bg-cyan-50 border-b border-cyan-200 p-4 flex justify-between items-center text-cyan-700">
                     ...
                     <div className="flex items-center gap-3">
                        ...
                        <button
                           ...
                           className="px-3 py-1.5 rounded-xl bg-white border border-cyan-200 text-cyan-700 font-black text-[10px] uppercase tracking-widest hover:bg-cyan-100 transition-colors shadow-sm"
                        >
                           ⬇ Export CSV
                        </button>
                     </div>
                  ...
                  <div className="overflow-y-auto p-4 flex flex-col gap-2 relative">
                     {participants.length === 0 ? (
                        ...
                        participants.map((p, i) => (
                           <div key={i} className="flex items-center gap-4 p-4 rounded-2xl bg-white border border-slate-200 hover:bg-slate-50 transition-colors group shadow-sm">
                              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-cyan-100 to-blue-100 border border-cyan-200 flex items-center justify-center text-slate-900 font-bold text-lg group-hover:scale-110 transition-transform">
                                 {(p.user?.name || p.externalName)?.charAt(0)}
                              </div>
                              <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2 shrink-0">
                                 ...
                                 <div className={`px-2.5 py-1 rounded text-[10px] font-black uppercase tracking-widest border ${p.type === 'member' ? 'bg-purple-50 text-purple-700 border-purple-200' :
