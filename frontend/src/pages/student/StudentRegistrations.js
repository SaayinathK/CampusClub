...
export default function StudentRegistrations() {
   ...
   return (
      <div className="min-h-screen p-6 md:p-10 font-sans text-slate-900 relative overflow-hidden bg-transparent">
         <div className="absolute top-0 left-1/4 w-[500px] h-[500px] bg-cyan-600/10 rounded-full mix-blend-screen filter blur-[120px] opacity-60 pointer-events-none" />
         ...
         <div className="max-w-5xl mx-auto relative z-10 space-y-8">
            ...
            <header className="surface-panel rounded-[2.5rem] p-8 md:p-10 border border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-2xl">
               ...
               <Link to="/student" className="theme-button-secondary px-5 py-2.5 rounded-full font-bold text-sm text-slate-700 border-slate-200 transition-all shadow-sm">
                  ← Dashboard
               </Link>
            </header>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
               {[
                  ...
                  <div key={s.label} className={`surface-card rounded-2xl p-5 border border-slate-200 shadow-sm`}>
            ...
            <div className="flex flex-wrap gap-2">
               {[
                  ...
                  <button
                     ...
                     className={`px-4 py-2 rounded-full font-bold text-xs uppercase tracking-widest transition-all border shadow-sm ${filter === f.value
            ...
            {loading ? (
               ...
               <div className="surface-panel rounded-3xl p-16 text-center border border-slate-200 shadow-2xl">
                  ...
                  <h3 className="text-2xl font-black mb-2 text-slate-900">No registrations</h3>
                  <p className="text-slate-500 text-sm mb-6">
                     {filter === 'all' ? "You haven't registered for any events yet." : `No ${filter} events found.`}
                  </p>
                  {filter === 'all' && (
                     <Link to="/events" className="px-6 py-3 rounded-full bg-cyan-50 border border-cyan-200 text-cyan-700 font-bold text-sm hover:bg-cyan-100 transition-colors shadow-sm">
                        Browse Events →
                     </Link>
                  )}
               </div>
               ...
               <div className="space-y-4">
                  {filtered.map((reg) => {
                     ...
                     return (
                        <div key={reg._id} className="surface-card rounded-2xl border border-slate-200 overflow-hidden hover:border-cyan-200 transition-all shadow-sm">
                           <div className="flex flex-col md:flex-row gap-0">
                              {/* Date column */}
                              <div className="md:w-24 bg-slate-50 flex md:flex-col items-center justify-center p-4 gap-2 md:gap-0 shrink-0 border-r border-slate-200">
                              ...
                              <div className="flex-1 p-5 flex flex-col md:flex-row gap-4">
                                 ...
                                 <div className="flex md:flex-col gap-2 shrink-0 md:items-end justify-end">
                                    <Link
                                       to={`/events/${reg._id}`}
                                       className="px-4 py-2 rounded-xl bg-cyan-50 border border-cyan-200 text-cyan-700 font-black text-[10px] uppercase tracking-widest hover:bg-cyan-100 transition-colors text-center shadow-sm"
                                    >
                                       View Event
                                    </Link>
                                    {isUpcoming && (
                                       <button
                                          ...
                                          className="px-4 py-2 rounded-xl bg-white border border-slate-200 text-slate-600 font-black text-[10px] uppercase tracking-widest hover:bg-rose-50 hover:text-rose-700 hover:border-rose-200 transition-colors shadow-sm"
                                       >
                                          Unregister
                                       </button>
                                    )}
